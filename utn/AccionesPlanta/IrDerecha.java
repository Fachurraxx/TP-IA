		...
		if (col != 8) {
	        ...
	        if (tableroValor == PlantaPerception.EMPTY_PERCEPTION || tableroValor == PlantaPerception.UNKNOWN_PERCEPTION ) {
	        	...
	        	if(posicionPlantaValor.contains("-")) {
	        		...
	        		estadoPlanta.setTableroEnPosicion(row, col+1, posicionPlantaValor);
	        	}
	        	else {
	            	estadoPlanta.setTableroEnPosicion(row, col+1, PlantaPerception.EMPTY_PERCEPTION);
	        	}
	        }
	        else{
	...
	public EnvironmentState execute(AgentState ast, EnvironmentState est) {
		...
		if (col != 8) {
	        ...
	        if (tableroValor == PlantaPerception.EMPTY_PERCEPTION || tableroValor == PlantaPerception.UNKNOWN_PERCEPTION ) {
	        	...
	        	if(posicionPlantaValor.contains("-")) {
	        		posicionPlantaValor = posicionPlantaValor.split("-")[1];
	        		estadoAmbiente.setTableroEnPosicion(row, col+1, posicionPlantaValor);
	        		estadoPlanta.setTableroEnPosicion(row, col+1, posicionPlantaValor);
	        	}
	        	else {
	            	estadoAmbiente.setTableroEnPosicion(row, col+1, PlantaPerception.EMPTY_PERCEPTION);
	            	estadoPlanta.setTableroEnPosicion(row, col+1, PlantaPerception.EMPTY_PERCEPTION);
	        	}
	        }
	        else{
